...
function [GAR,FAR,EER]=ezroc3_MI(H,T,plot_stat,headding,printInfo)%,rbst
t1=min(min(min(H)));
t2=max(max(max(H)));
num_subj=size(H,1);
...
elseif (nargin==2 || nargin==3 || nargin == 4 || nargin == 5),   %Regular, 1-class-vs-rest ROC, and maybe 3D or no plot
    ...
    for t=(t1-stp):stp:(t2+stp),    %span the limits by a smidgeon to get a full curve
        ...
        TP=sum(and(id,T)); %TP
        FN=sum(and(not(id),T));   %FN
        
        TN=sum(and(not(id),not(T)));   %TN
        FP=sum(and(id,not(T)));   %FP
        
        for i=1:40
            True_Accept(i) = (TP(:,i)+TP(:,(i+1))+TP(:,(i+2))+TP(:,(i+3))+TP(:,(i+4)))/5
            False_Reject(i) = (FN(:,i)+FN(:,(i+1))+FN(:,(i+2))+FN(:,(i+3))+FN(:,(i+4)))/5
            True_Reject(i) = (TN(:,i)+TN(:,(i+1))+TN(:,(i+2))+TN(:,(i+3))+TN(:,(i+4)))/5
            False_Accept(i) = (FP(:,i)+FP(:,(i+1))+FP(:,(i+2))+FP(:,(i+3))+FP(:,(i+4)))/5
        end
